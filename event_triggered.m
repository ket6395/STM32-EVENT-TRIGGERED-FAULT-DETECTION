% ==========================================================
% STM32-Based Event Triggered Monitoring System
%
...
%
% Objective:
% Validate event-triggered execution before STM32 deployment.
% ==========================================================
clc;
clear;
close all;
...
event_execution_rate = (control_count/periodic_executions)*100;
execution_reduction =100 - event_execution_rate;
fprintf('\n');
...
if length(trigger_times) > 1
  inter_event_times = diff(trigger_times);
    ...
    avg_inter_event_time =mean(inter_event_times);
...
plot(t(trigger_events==1),e_norm_str(trigger_events==1), 
...
%% State Comparison
figure('Color','w');
plot(t,x_str(1,:),'b','LineWidth',1.5);
hold on;
plot(t,x_p_store(1,:), ...
    'r--','LineWidth',1.5);
xline(1,'m--','Disturbance Applied');
